...
export default function BuyElectricity() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-medium" id="_buy_electricity_page">
        You can buy your electricity here
      </h1>
      {error !== "" && (
      ...
      {isSuccess && (
        <div
          ...
          id="SuccessMessage"
        >
          Successfully Token bought :{token} ..
        </div>
      )}
      <div>
        <Input
        ...
        <Input
          type="number"
          placeholder="Amount"
          data={{ st: amount, sts: setAmount }}
        />

        <Button
          action="buyElectricity"
          title="Buy Electricity"
          onClick={buyElectricty}
        />
      </div>
    </div>
  );
